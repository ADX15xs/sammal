...
| internal/skill/skill.go | SKILL.md frontmatter 解析为逐行 key: value：不支持 YAML 块标量/多行值/引号转义，多行 description 会被静默忽略（正文完整保留） | 遇到首个解析不符合预期的真实 skill 格式时扩展 parse | 按需（首个解析不符预期的真实 skill 出现时） |
...
| internal/tui/scroll.go | bubbletea v2.0.9 insertAbove 两个缺陷的应用侧对策（渐进落盘 + 宽度归一化 + 巨行硬拆）覆盖了主路径，残余边界未堵：① 模型输出含 `\t` 时 ansi.StringWidth 与真实终端 8 列制表位宽度分叉，行数估算可漂移；② 含 ANSI 的巨行硬拆可能在转义序列中间切分（字素簇把 ESC 当独立簇）；③ 落盘 Println 与 ticker flush 分属事件循环与独立 goroutine（仅 s.mu 串行、顺序可交错），提高打印频率后交错 artifact（turnTick 注释记录过空行现象）的触发面变大但未实测；④ 理论上定稿补余的多批 Sequence 可与下一步事件打印竞序（人手操作不可达）。宽度归一化与分批对估算公式实现解耦，上游修复落地后零改动。决策：不对上游提 issue、不跟随其修复节奏——对策不被上游行为变化影响，上游缺陷仅作外部事实记录（决策记录 2026-09-05） | 三终端实机冒烟长回答 + 流中断重连；bubbletea / x-ansi 升级后重审 scroll_test.go 的估算公式快照（归一化在公共正确区，测试对齐的是 v2.0.9 行为）；①② 出现实际错乱时在 printScroll 入口展开 tab / 剥离打印文本 ANSI | 首个实机复现残余项或出现实际错乱时 |
